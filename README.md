...
- [x] GUI Parser: It based on Google OCR and PyAutoGUI for extracting the element grounding information (i.e., GUI Info).

- [x] State-Aware Planner: Accept screenshot and instructional video to plan.

- [x] Planner-Critic: Correct the initial plan generated by Planner.
- [x] Step-Check: Check the task completion and redundancy with various output status (e.g., `<Modify>`, `<Pass>`, `<Continue>`, `<Finished>`). We also implement a LLM-driven region search module to locate the target elements.

- [x] Actor: Turn the action description to executed code (e.g., click(100, 200)).

- [x] Actor-Critic:

- [x] Input with instructional video:

- [x] Input without instruction video:
